...
trait AkkaInjectable extends Injectable {
  ...
  protected def injectActorRef[T](implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(injector, ct, tt, nn))

  protected def injectActorRef[T](name: String)
                                 (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(injector, ct, tt, nn), name)
  ...
  protected def injectActorRef[T](identifiers: Identifier*)
                                 (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(identifiers: _*)(injector, ct, tt, nn))

  protected def injectNamedActorRef[T](identifiers: Identifier*)
                                      (name: String)
                                      (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(identifiers: _*)(injector, ct, tt, nn), name)
  ...
  protected def injectActorRef[T](constraints: => InjectConstraints[T])
                                 (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(constraints)(injector, ct, tt, nn))

  protected def injectNamedActorRef[T](constraints: => InjectConstraints[T])
                                      (name: String)
                                      (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    s.get.fold(identity, _.system).actorOf(injectActorProps(constraints)(injector, ct, tt, nn), name)
}
...
trait OpenAkkaInjectable extends AkkaInjectable {
  ...
  override def injectActorRef[T](implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectActorRef[T](s, injector, ct, tt, nn)

  override def injectActorRef[T](name: String)
                                (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectActorRef[T](name)(s, injector, ct, tt, nn)
  ...
  override def injectActorRef[T](identifiers: Identifier*)
                                (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectActorRef[T](identifiers: _*)(s, injector, ct, tt, nn)

  override def injectNamedActorRef[T](identifiers: Identifier*)
                                     (name: String)
                                     (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectNamedActorRef[T](identifiers: _*)(name)(s, injector, ct, tt, nn)
  ...
  override def injectActorRef[T](constraints: => InjectConstraints[T])
                                 (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectActorRef[T](constraints)(s, injector, ct, tt, nn)

  override def injectNamedActorRef[T](constraints: => InjectConstraints[T])
                                     (name: String)
                                     (implicit s: ActorSystem Or ActorContext, injector: Injector, ct: ClassTag[T], tt: TypeTag[T], nn: NotNothing[T]): ActorRef =
    super.injectNamedActorRef[T](constraints)(name)(s, injector, ct, tt, nn)
}